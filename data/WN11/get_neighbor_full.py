...
def safe_name(idx: str, ent2txt_map: Dict[str, str]) -> str:
    return ent2txt_map.get(idx, idx)
...
def make_1hop_sentences_for_WN11(s: str, df: pd.DataFrame, ent2txt_map: Dict[str, str]) -> List[str]:
    name_s = safe_name(s, ent2txt_map)
    subset = df[df['entity'] == s]
    sentences = [f"{name_s} {rel} {t}" for rel, t in zip(subset['relation'], subset['tail'])]
    seen, uniq = set(), []
...
def make_2hop_sentence_pair_WN11(s: str, w: str, t: str, relations_dir, ent2txt_map: Dict[str, str]) -> str:
    r1 = pick_relation(relations_dir, s, w) or "relation"
    r2 = pick_relation(relations_dir, w, t) or "relation"
    sen1 = f"{safe_name(s, ent2txt_map)} {r1} {safe_name(w, ent2txt_map)}"
    sen2 = f"{safe_name(w, ent2txt_map)} {r2} {safe_name(t, ent2txt_map)}"
    return f"{sen1}; {sen2}."
...
def main():
    ...
    rows = []
    for s in all_nodes:
        one_hop_sents = make_1hop_sentences_for_WN11(s, df, ent2txt)
        ...
        for (t, aa, bc_t, comb) in top2:
            ...
            if w is None:
                continue
            sent = make_2hop_sentence_pair_WN11(s, w, t, relations_dir, ent2txt)
            two_hop_sents.append(sent)
            two_hop_scores.append(f"{t}|aa={aa:.6f}|bc_t={bc_t:.6f}|combined={comb:.6f}|via={w}")
    ...
    print(f"CSV file '{csv_file}' has been created for WN11 with 1-hop sentences and top-{topk_second} 2-hop sentences.")
